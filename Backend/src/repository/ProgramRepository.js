...
repository.readProgram = async (req) => {
  ...
  try {
    ...
    let idPrograms = (await clt.query(`
      SELECT id_program
      FROM _program 
      WHERE id_user = $1
    ...
    for (let index = 0; index < idPrograms.length; index++) {
      volume = await repository.getVolume(idPrograms[index].id_session, req.user.id)
      console.log("Volume =>, \n", volume)
      var result = await clt.query(sql, [req.user.id])
      //console.log("RESULT ROWS : ", result.rows)
      if (result.rowCount != 0)
        program_list.push(new Program(result.rows[index].id_session, result.rows[index].name, result.rows[index].program_goal_name, result.rows[index].session_count, volume))
    }
...
repository.readDetailProgram = async (req) => {
  ...
  try {
    ...
    var program_goal_result = await clt.query(program_sql, [
      req.user.id,
      req.params.id_program,
    ]);
...
repository.addProgram = async (req) => {
  let sqlAddProgram =
    "INSERT INTO _program (id_user, id_program_goal, name, creation_date, last_update) VALUES ($1, $2, $3, $4, $5)";
  try {
    await clt.query(sqlAddProgram, [
      req.user.id,
      req.body.id_program_goal,
      req.body.name,
      new Date(),
      new Date(),
    ]);
    let sqlLastProgramCreated =
      "SELECT id_program FROM _program WHERE id_user = $1 ORDER BY creation_date DESC";
    ...
    req.body.sessions.forEach(async (session) => {
      ...
      let insertInProgramSession =
        "INSERT INTO _program_session (id_user, id_user_1, id_program, id_session, place) VALUES ($1, $2, $3, $4, $5)";
      await clt.query(insertInProgramSession, [
        req.user.id,
        req.user.id,
        getIdProgram.rows[0].id_program,
        parsed_session.id,
        parsed_session.place,
      ]);
    });
    return 201;
  } catch (error) {
...
repository.updateProgram = async (req) => {
  ...
  try {
    ...
    sql =
      "UPDATE _program SET name = $1, last_update = $2, id_program_goal = $3 WHERE id_program = $4 AND id_user = $5";
    await clt.query(sql, [
      req.body.name,
      new Date(),
      result.rows[0].id_program_goal,
      req.params.id_program,
      req.user.id,
    ]);
    sql =
      "DELETE FROM _program_session WHERE id_user = $1 AND id_user_1 = $2 AND id_program = $3";
...
repository.getVolume = async (id_program, id_user) => {
  ...
  try {
    let res = 0
    let id_session_list = await clt.query(
      `
          SELECT ps.id_session
          FROM _program p
          JOIN _program_session ps on p.id_program = ps.id_program
          WHERE ps.id_program = $1 AND p.id_user = $2 
          ORDER BY last_update DESC
      `,
      [id_program, id_user])
    for (let idx = 0; idx < id_session_list.rows.length; idx++) {
      const elemt = id_session_list.rows[idx]["id_program"];
      console.log("ELEMNT SSN", elemt)
      let exercisesVolumes = await SessionRepository.getVolume(elemt, id_user)

      res += parseInt(exercisesVolumes.volume)
